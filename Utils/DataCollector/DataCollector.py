...
class DataCollector:
    ...
    def Buffer(self):
        if len(self.chunk_EEG) <= self.bufferPoint_:
            return
        else:
            self.chunk_EEG = self.chunk_EEG[self.bufferPoint:]
    ...
    def dataGet(self, NeedTime):
        ...
        data_preprocEEG = Stage_preProcess(self.b, self.a, self.chunk_EEG, self.factor)
    ...
    def dataGet_All(self):
        self.Buffer()
        length = min(len(self.chunk_EEG), len(self.chunk_EOG), len(self.chunk_EMG))
        data = None
        if self.useChannel == 1:
            data = [self.chunk_EEG[:length]]
        elif self.useChannel == 2:
            data = [self.chunk_EEG[:length], self.chunk_EOG[:length]]
        elif self.useChannel == 3:
            data = [self.chunk_EEG[:length], self.chunk_EOG[:length], self.chunk_EMG[:length]]
        return np.array(data)

    def dataGet_Single(self, NeedTime):
        ind = int(-NeedTime * self.fs)
        data = self.chunk_EEG[ind:]
        return np.array(data)
