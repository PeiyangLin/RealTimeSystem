                ...
                for i in range(index, len(data_tail_DownState) - 1):
                    ...
                    if data_tail_DownState[i] * data_tail_DownState[i + 1] <= 0 and data_tail_DownState[i] <= 0:
                        ...
                        if UpState_max >= self.pos_thresh:
                            ...
                            # plt.plot(t, data_filt[-1500:])
                            # plt.hlines(y=self.neg_thresh, xmin=-1, xmax=4, colors="r")
                            # plt.hlines(y=self.pos_thresh, xmin=-1, xmax=4, colors="r")
                            # plt.hlines(y=0, xmin=-1, xmax=4, colors="r", linestyles="dashed")
                            # plt.xlim([0,3])
                            # plt.savefig("LOG/01-2023_10_24-ZhouJianyang-Male/IMG/%s.png" % time.strftime('%Y-%m-%d-%H-%M-%S', time.localtime()))
                            # plt.close()

                            now_plot = time.time()

                            print("filter, Use Time: %.3fms" % ((now_filt - now) * 1000))
                            print("down, Use Time: %.3fms" % ((now_down - now_filt) * 1000))
                            print("up, Use Time: %.3fms" % ((now_up - now_down) * 1000))
                            print("plot, Use Time: %.3fms" % ((now_plot - now_up) * 1000))
                        break
